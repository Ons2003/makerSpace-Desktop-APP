...
public class MakerspaceTimeline_Controller implements TimeEventPanel.ITimeEventListener {

    public static MakerspaceTimeline_Controller instance;
    ...
    private void SetupNewEventPopup() throws IOException {
        ...
        FXMLLoader fxmlLoader = new FXMLLoader(HelloApplication.class.getResource("NewTimeEvent-View.fxml"));
    ...
    public void DisplayTimeEventData(TimeEvent timeEvent) {
        ...
        eventCreator_label.setText(timeEvent.creation.creator.name);
    ...
    public void ChangeAccount(){
        System.out.println((AccountsManager.loggedAccount == null) + " h");
        if(AccountsManager.loggedAccount.role == Account.UserRole.SuperUser)
        {
            AccountsManager.loggedAccount = AccountsManager.GetAccountOfName("ahmed khemiri");
        }else if(AccountsManager.loggedAccount.role == Account.UserRole.Student)
        {
            AccountsManager.loggedAccount = AccountsManager.GetAccountOfName("yassine chaouch");
        }else if(AccountsManager.loggedAccount.role == Account.UserRole.Professor)
        {
            AccountsManager.loggedAccount = AccountsManager.GetAccountOfName("youssef chebil");
        }
        loggedAccountName_button.setText(AccountsManager.loggedAccount.role.toString());
        loggedAccountName_Label.setText(AccountsManager.loggedAccount.name);
        timetable.RetrieveEventsFromDatabase(this);
    }
    ///endregion
}